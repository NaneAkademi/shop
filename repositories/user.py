...
class UserRepository:
    ...
    @staticmethod
    async def create(user_dto: UserCreateDTO, session: AsyncSession | Session):
    ...
    @staticmethod
    async def update_balance(user_id: int, amount: float, session: AsyncSession | Session):
        stmt = (update(User)
                .where(User.id == user_id)
                .values(top_up_amount=User.top_up_amount + amount))
        await session_execute(stmt, session)
