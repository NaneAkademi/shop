...
class AdminService:
    ...
    async def get_statistics_menu() -> tuple[str, InlineKeyboardBuilder]:
        ...
        kb_builder.button(text=Localizator.get_text(BotEntity.ADMIN, "download_project"),
                          callback_data=StatisticsCallback.create(4))
        kb_builder.adjust(1)
        kb_builder.row(AdminConstants.back_to_main_button)
    ...
    @staticmethod
    async def create_project_zip(callback: CallbackQuery) -> str:
        """Create a zip file of the entire project and send it directly via Telegram"""
        try:
                    ...
                    for file in files:
                        ...
                        if not should_exclude(file_path):
                            ...
                            try:
                                ...
                                logging.error(f"Error adding {file_path} to zip: {e}")
            ...
            if file_size_mb > 50:
                ...
                return Localizator.get_text(BotEntity.ADMIN, "project_zip_error").format(
                    error="Zip dosyası Telegram limiti olan 50MB'ı aşıyor. Proje çok büyük."
                )

            # Send zip file directly to admin
            zip_file_input = FSInputFile(zip_filename)
            
            await callback.message.answer_document(
                zip_file_input,
                caption=Localizator.get_text(BotEntity.ADMIN, "project_zip_created").format(
            ...
            return "✅ Proje zip dosyası başarıyla oluşturuldu ve gönderildi!"
            ...
            return Localizator.get_text(BotEntity.ADMIN, "project_zip_error").format(error=str(e))